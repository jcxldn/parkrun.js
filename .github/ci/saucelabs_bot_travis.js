...
async function uploadB2() {
	...
	await asyncForEach(files, async file => {
		...
		const upload = await retry((retry, number) => {
			...
			return b2
				.uploadFile({
					...
					fileName: "parkrunjs_/saucelabs_img/" + file.filename.replace(".json", ".png"),
					mime: "image/png",
					data: Buffer.from(data.image.replace(/^data:image\/png;base64,/, ""), "base64"),
				})
				.catch(err => {
					retry(err);
				});
		...
		console.log("https://dl-f0.jcx.ovh/file/jcx-pub-dl/" + upload.data.fileName);
		console.log(file.filename.split(".")[0]); // ID
		console.log("Uploaded!");

		output.push(Object.assign(data, { image: undefined, id: file.filename.split(".")[0] }));
	});
...
function createSubset(item) {
	return `<details>
<summary>${item.num_failed == 0 ? ":heavy_check_mark:" : ":x:"} ${item.browser}@${
		item.version
	} (${item.platform.toString().toLowerCase()})</summary>
<br>
...
<img src="https://dl-f0.jcx.ovh/file/jcx-pub-dl/parkrunjs_/saucelabs_img/${item.id}.png"/>
</details>`;
}
